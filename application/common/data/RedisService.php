...
class RedisService
{
	...
	//初始化
	public function __construct($host,$port,$dbname,$password,$timeout,$type)
	{
		$this->_HOST = $host;
		$this->_PORT = $port;
		$this->_DBNAME = $dbname;
		...
		if (!isset($this->_REDIS)) {
			...
			$this->connect($this->_HOST, $this->_PORT, $this->_TIMEOUT, $this->_DBNAME, $this->_PASSWORD, $this->_CTYPE);
		}
	}
	 ...
	 * 连接redis服务器
	 */
	private function connect($host,$port,$timeout,$dbname,$password,$type)
	{
		switch ($type) {
			case 1:
